...
def save_snapshot():
    ...
    try:
        ...
        detected_labels = [detected_objects[int(cls)] for cls in results[0].boxes.cls.cpu().numpy()]

        phone_detected = any(label.lower() in ['cell phone', 'phone'] for label in detected_labels)
        ...
        with open(log_path, 'a') as log:
            ...
            if len(face_encodings) > 1:
                log.write(f"{timestamp} - ALERT: Multiple faces detected in {filename}\n")
            ...
            if is_match:
                log.write(f"{timestamp} - OK: Face matched with reference in {filename}\n")
            else:
                log.write(f"{timestamp} - ALERT: Face mismatch in {filename}\n")

    except Exception as e:
        with open(log_path, 'a') as log:
            log.write(f"{timestamp} - ERROR: Exception during snapshot processing: {str(e)}\n")
...
def check_blink():
    ...
    return jsonify({'status': 'success', 'message': 'Face detected (blink check placeholder)'})
...
def mic_audio():
    ...
    try:
        ...
        log_path = os.path.join(UPLOAD_FOLDER, 'events.log')
        with open(log_path, 'a') as log:
            if audio_volume > 500:  # Threshold to detect if audio is silent or noisy
                log.write(f"{timestamp} - ALERT: Non-silent audio detected (volume={audio_volume})\n")
            else:
                log.write(f"{timestamp} - OK: Silent background (volume={audio_volume})\n")
    except Exception as e:
        # Log audio processing errors, but don't fail
